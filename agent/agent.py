...
def agent_loop(user_input):
    ...
    global longterm_mem, dynamic_mem, history, isFirstTime
    recent_history = history[-4:]
    state = build_state()
    # 第一次循环才读真正的长文档，后面都读小的
    if not isFirstTime :
        messages = [
            {"role": "system", "content": real_soul+state},
    ...
    while True:
        ...
        if choice.finish_reason == "stop":
            ...
            def background_memory_task():
                global longterm_mem, dynamic_mem
                longterm_mem, dynamic_mem = update_memory(
